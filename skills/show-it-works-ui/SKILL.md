...
4. Stop the recording as soon as the take is written. A recorder does not release anything when the video file appears: the browser it drove, its renderer and GPU child processes, the profile directory it opened and any local server it started all stay alive until something closes them explicitly. Close them from a `finally` or an equivalent cleanup path so that a scenario which fails halfway tears down as thoroughly as one that succeeds, and let the process exit instead of holding it open for the next take — otherwise a session that records a few times leaves several browsers loading the machine. Throwaway probe scripts count: they launch the same browser without recording, and an abandoned probe costs as much as an abandoned take. After a run that had to be interrupted, look for the surviving process and stop it before recording again.
5. Capture only the relevant application window or browser context. Do not record credentials, terminals, local files, unrelated browser tabs, or other private data: anyone with the viewer link sees whatever was in frame. When a sign-in is required, ask the user to authenticate and start recording only once the authenticated page is visible. Never request credentials in chat or write them into a generated script.
6. If no safe recording method is available, state that video evidence could not be produced and provide the exact blocker. Do not claim visual verification.
